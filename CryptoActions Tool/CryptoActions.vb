...
Module CryptoActions
    ...
    Function GetDeriveBytes(password As String, size As Integer) As Byte()
        If ((String.IsNullOrWhiteSpace(password)) OrElse (password.Length < 8)) Then
            MsgBox("Error en el módulo 'GetDeriveBytes'" & vbCrLf & "La llave criptográfica debe tener más de 8 caracteres", MsgBoxStyle.Critical, "CryptoActions")
        End If
        If ((size < 1) OrElse (size > 128)) Then
            MsgBox("Error en el módulo 'GetDeriveBytes'" & vbCrLf & "El tamaño tiene que estar comprendido entre 1 y 128.", MsgBoxStyle.Critical, "CryptoActions")
        End If
